...
module.exports = function({ $std_Json }) {
  class $Closure6 {
    ...
    async handle() {
      const obj = ({"a": 1,"b": [3, 7, 9]});
      const entries = (Object.entries(obj));
      const keys = (Object.keys(obj));
      const values = (Object.values(obj));
      {console.log(String.raw({ raw: ["", ""] }, entries))};
      {console.log(String.raw({ raw: ["", ""] }, JSON.stringify((await entries.at(0)))))};
      {console.log(String.raw({ raw: ["", ""] }, JSON.stringify((await entries.at(1)))))};
